...
void selectButton2::draw(QPainter *painter) const
{
    ...
    if(level==1)
    {
        painter->drawText(QRect(m_pos.x(),m_pos.y(),200,50),QString("update: %1").arg(towerupdate1Cost));
    }
    else
    {
        painter->drawText(QRect(m_pos.x(),m_pos.y(),200,50),QString("update: %1").arg(towerupdate2Cost));
    }
    painter->drawText(QRect(m_pos.x(),m_pos.y()+50,200,25),QString("remove"));
    painter->restore();
}
